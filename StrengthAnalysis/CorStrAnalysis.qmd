...
full_df <- read.csv("data/analysis_data/FullSample_25.csv", header= TRUE)
...
sample_n200_t25 <- read.csv("data/analysis_data/ReplicateSample_n200.csv",
                            header= TRUE)
...
logit_gam6 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/logit_gam6.rds")
...
#### 11) Model 8: Spatial Structure and covariates
...
logit_gam8 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/logit_gam8.rds")
...
logit_gam9 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/logit_gam9.rds")
...
#write.csv(testsample_n0, "C:\\Users\\shil5756\\Documents\\ConnectivityData_master\\Data\\Dataframes2\\testsample_ks_april_n0.csv", row.names = FALSE)

#to replicate my sample again:
testsample_n0 <- read.csv("C:\\Users\\shil5756\\Documents\\ConnectivityData_master\\Data\\Dataframes2\\testsample_ks_april_n0.csv",header= TRUE)
...
#replicate my sample
Contin_gam_guass <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Contin_gam_guass.rds")
...
#replicate my sample
Contin_gam_gamma <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Contin_gam_gamma.rds")
...
#replicate my sample
Contin_gam_tweed <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Contin_gam_tweed.rds")
...
Gam_tw0_test <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Gam_tw0_test.rds")
...
Gam_tw1_test <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Gam_tw1_test.rds")
...
Gam_tw2_test <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Gam_tw2_test.rds")
...
The qq plot of residuals does seem to follow normal residuals, but is now we more seriosuly violating the assumption of homoskedasticity.
...
#save
#saveRDS(glmm1_test, "~/ConnectivityData_master/Data/SavedModels2/Glmm1_test.rds")
glmm1_test <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Glmm1_test.rds")
...
#save
#saveRDS(glmm2_test, "~/ConnectivityData_master/Data/SavedModels2/Glmm2_test.rds")
glmm2_test <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Glmm2_test.rds")
...
```{r}
knitr::include_graphics("~/ConnectivityData_master/Data/Images/HurdleP2_finalmodel_showcase2.png", dpi = 300)
```
...
sample_n100_t25_n0 <- read.csv("C:\\Users\\shil5756\\Documents\\ConnectivityData_master\\Data\\Dataframes2\\sample_n100_t25_n0_ksApril.csv",header= TRUE)
...
Gam1_n100 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Gam1_n100.rds")
...
Gam2_n100 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Gam2_n100.rds")
...
You could also try a tensor product smooth:

```{r}
Gam3_n100 <- gam(Vulnerability ~ logCrStr + Precipitation + Habitat_Suitability + Start_Distance + te(x,y) + s(Species,bs="re"), family = tw(link = "log"), data = sample_n100_t25_n0)

summary(Gam3_n100)
AIC(Gam3_n100)

#view residual fitted:
DharmaSim_Gam3_n100 <- simulateResiduals(fittedModel = Gam3_n100, 
                                            plot = F)
DharmaSim_Gam3_n100_BySp = recalculateResiduals(DharmaSim_Gam3_n100, 
                                         group = sample_n100_t25_n0$Species)

plot(DharmaSim_Gam3_n100)
plot(DharmaSim_Gam3_n100_BySp)

#to use the montecarlo moran test, which is more robust, we need to make a spatial weights matrix
Gam3_n100_deviance <- residuals(Gam3_n100, type = 'deviance')
moran.mc(Gam3_n100_deviance, listw = lw3, nsim = 999, alternative = "greater")

#plot variogram 
dummytestsample <- sample_n100_t25_n0

# Add residuals to your dummy dataframe
dummytestsample$Gam3_n100_deviance <- Gam3_n100_deviance

# Calculate the empirical variogram
variogram_Gam3_n100 <- variogram(Gam3_n100_deviance ~ 1, 
                                 locations = ~x + y,
                                 data = dummytestsample,
                              cutoff= 250000, width= 250000/30)

plot(variogram_Gam3_n100, 
     main = "Variogram of Gam3 Residuals; Y~X + full + re(Species) + Niche(xy)", 
     pch = 16, col = "blue")

#extract nugget and sill
#approcimate the nugget by getting the semivairance at the lowest distance bin
variomodel_Gam3_n100 <- fit.variogram(variogram_Gam3_n100, 
                                 model = vgm(psill = 1, "Exp", 
                                             range = 500000, 
                                             nugget = 0.1))

plot(variogram_Gam3_n100, model = variomodel_Gam3_n100, 
     main = "Variogram of Gam3 Residuals; Y~X + full + re(Species) + Niche(xy)")

nugget_Gam3 <- variomodel_Gam3_n100$psill[1]
sill_Gam3 <- variomodel_Gam3_n100$psill[1] + variomodel_Gam3_n100$psill[2]
non_sp_variaiton_Gam3 <- (nugget_Gam3/sill_Gam3)*100
spatialvariaiton_Gam3 <- 100-non_sp_variaiton_Gam3

#build spatial inference df
SACtable_Gam3 <- data.frame(Nugget = nugget_Gam3, 
                        Sill = sill_Gam3, 
                        Percentage_Variatin_Spatial = spatialvariaiton_Gam3)
SACtable_Gam3

#build CI conclusion table
Gam3_summary <- summary(Gam3_n100)

lowerbound_Gam3 <- Gam3_summary$p.coeff["logCrStr"] - 1.96 *    
                Gam3_summary$se["logCrStr"]
upperbound_Gam3 <- Gam3_summary$p.coeff["logCrStr"] + 1.96 * 
                Gam3_summary$se["logCrStr"]

#and put it all in a nice dataframe:
Gam3_CI <- data.frame(Lower_CI = lowerbound_Gam3, 
                        Slope = Gam3_summary$p.coeff["logCrStr"], 
                        Upper_CI = upperbound_Gam3)
Gam3_CI
```
...
glmm1_n100 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/Glmm1_n100.rds")
...
```{r}
#read in model, due to long run times
glmm2_n100 <- readRDS("~/ConnectivityData_master/Data/SavedModels2/glmm2_n100.rds")
...
Although we can view the relationship with no transformations, which seems intuitive:
...
```{r}
knitr::include_graphics("~/ConnectivityData_master/Data/Images/natlog_breaks.png", dpi = 300)
```
...
ggplot(data = sample_n100_t25_n0, 
  ...
  labs(x = "Log Corridor Strength", y = "Climate Vulnerability", 
       title = "GLMM2 on larger sample, predictions") +
  theme_light() +
...
```{r}
knitr::include_graphics("~/ConnectivityData_master/Data/Images/GLMM2_natbreaks_demo.png", dpi = 300)
```
...
GAM3predictions_n100 <- predict(Gam3_n100, type = "response")
GAM2predictions_n100 <- predict(Gam2_n100, type = "response")
GLMM1predictions_n100 <- predict(glmm2_n100, type = "response")
GLMM1predictions_values_n100 <- as.numeric(GLMM1predictions_n100)
GLMM2predictions_n100 <- predict(glmm2_n100, type = "response")
...
new_data_n100$predicted_vuln_gam3 <- predict(Gam3_n100, newdata = new_data_n100, type = "response")
...
ggplot(data = sample_n100_t25_n0, 
       aes(x = logCrStr, y = Vulnerability)) +
  geom_point(size= 0.75, alpha= 0.05) +
  geom_smooth(aes(y = GAM3predictions_n100), color = "#009933", size = 1,  se=F, linetype = "dashed") +  
      geom_smooth(aes(y = new_data_n100$predicted_vuln_gam3), 
              color = "#009933", size = 1.6, se=F) +
  labs(x = "Log Corridor Strength", y = "Climate Vulnerability", 
       title = "GAM3 predictions") +
  theme_light() +
...
ggplot(data = sample_n100_t25_n0, 
       aes(x = logCrStr, y = Vulnerability)) +
  geom_point(size= 0.75, alpha= 0.05) +
  geom_smooth(aes(y = GAM2predictions_n100), color = "#00ff33", size = 1,  se=F, linetype = "dashed") +  
        geom_smooth(aes(y = new_data_n100$predicted_vuln_gam2), 
              color = "#00ff33", size = 1.6, se=F) +
  labs(x = "Log Corridor Strength", y = "Climate Vulnerability", 
       title = "GAM2 predictions") +
  theme_light() +
  coord_cartesian(ylim=c(0, 700000))
...
```{r}
save.image(file = "~/ConnectivityData_master/Data/GlobalEnv/march31_globenv.RData")
```
